...
const corePrefetchRoutes = [...navigation.map((item) => item.href), '/settings']
...
export function Sidebar({ isCollapsed = false, toggleCollapse }: SidebarProps) {
  ...
  useEffect(() => {
    ...
    const prefetchRoutes = () => {
      for (const route of corePrefetchRoutes) {
  ...
  return (
    ...
    >
      <div className={cn('flex h-16 items-center border-b border-border px-6', isCollapsed ? 'justify-center px-0' : 'gap-2')}>
        {!isCollapsed ? (
          <Image src={logoSrc} alt="Networkly" width={120} height={40} className="object-contain" priority />
        ) : (
          <Image src={logoMiniSrc} alt="Networkly" width={40} height={40} className="object-contain" priority />
        )}
      </div>
